...
for i in species:
    ctct_pairs = pd.read_csv(f'/beegfs/data/hpcws/ws1/popov-popov_gnomad/GENA_loop_predict/ctcf_pairs/{i}_CTCF_pairs_random_control.bedpe', sep='\t', header=None, names=['chrom1', 'start1', 'end1', 'chrom2', 'start2', 'end2'])
    ...
    file = h5py.File(f'/beegfs/data/hpcws/ws1/popov-popov_gnomad/GENA_loop_predict/for_GENA/{i}_tads_random_control.hdf5', 'a')
    for num, row in tqdm.tqdm(ctct_pairs.iterrows()):
        ...
        if observed.empty:
            ...
            start_multiple, end_multiple = find_multiple(observed['end1'].values, observed['start2'].values)
